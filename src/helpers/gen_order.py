...
def gen_charge(customer, products, created_at, stream = False):
    paid_at = created_at.add(
        minutes=randint(0, 15),
        seconds=randint(5, 60)
        )
    ...
    def generate(self, dt = None):
        ...
        if dt is None:
            fake_dt = fake.date_between(
                start_date = pendulum.from_format(customer["created_at"], 'YYYY-MM-DD', tz='America/Fortaleza'), 
                end_date = '-1d'
                )
            
            created_at = pendulum.from_format(str(fake_dt), 'YYYY-MM-DD', tz='America/Fortaleza').add(
                hours=randint(0, 23),
